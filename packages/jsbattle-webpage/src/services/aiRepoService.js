...
export const getScript = async (name, namespace) => {
  ...
  return JSON.parse(JSON.stringify(scriptMap[name]));
};
...
export const renameScript = async (newValue, oldValue, namespace) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
...
export const updateScript = async (name, code, namespace) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
  let storedScripts = localStorage.getItem(namespace);
  let scriptMap = storedScripts ? JSON.parse(storedScripts) : {};
  if(!scriptMap[name]) {
...
export const deleteScript = async (name, namespace) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
  let storedScripts = localStorage.getItem(namespace);
  let scriptMap = storedScripts ? JSON.parse(storedScripts) : {};
  let newMap = {};
