...
function getStorageName(namespace) {
  if(namespace == 'user') return 'scriptMap';
  if(namespace == 'challenge') return 'challengeLibrary.scriptMap';
  return namespace;
}

function isNameAllowed(name, namespace) {
  if(name.length < 3) return false;
  let storedScripts = localStorage.getItem(getStorageName(namespace));
  let scriptMap = storedScripts ? JSON.parse(storedScripts) : {};
  return !scriptMap[name];
}

function createScriptWithName(name, namespace) {
  let storedScripts = localStorage.getItem(getStorageName(namespace));
  let scriptMap = storedScripts ? JSON.parse(storedScripts) : {};
  if(namespace == 'user' && Object.keys(scriptMap).length >= 7) {
  ...
  localStorage.setItem(getStorageName(namespace), JSON.stringify(scriptMap));
  return script;
}
...
export const createScript = async (namespace, suggestedName) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
  let name = suggestedName || getRandomScriptName(namespace);
  let retry = 0;
  while(!isNameAllowed(name, namespace)) {
    name = getRandomScriptName(namespace);
    retry++;
    if(retry > 100) {
      throw "Cannot find unique name for the script";
    }
  }
  return createScriptWithName(name, namespace);
};

export const renameScript = async (newValue, id, namespace) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
  ...
  if(existsScript(newValue, namespace)) {
  ...
  localStorage.setItem(getStorageName(namespace), JSON.stringify(newScriptMap));
  return {
    id: 'local_' + newValue,
    scriptName: newValue
  };
...
export const getScriptNameList = async (namespace) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
  let storedScripts = localStorage.getItem(getStorageName(namespace));
  let scriptMap = storedScripts ? JSON.parse(storedScripts) : {};
  let result = [];
...
export const updateScript = async (id, code, namespace) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
  let name = id.replace(/^local_/, '');
  let storedScripts = localStorage.getItem(getStorageName(namespace));
  let scriptMap = storedScripts ? JSON.parse(storedScripts) : {};
...
export const deleteScript = async (id, namespace) => {
  await new Promise((resolve) => setTimeout(resolve, serviceDelay));
  let name = id.replace(/^local_/, '');
  let storedScripts = localStorage.getItem(getStorageName(namespace));
  let scriptMap = storedScripts ? JSON.parse(storedScripts) : {};
  ...
  localStorage.setItem(getStorageName(namespace), JSON.stringify(newMap));
};

export const getOrCreateScript = async (name, namespace) => {
  let exists = existsScript(name, namespace);
  if(!exists) {
    await new Promise((resolve) => setTimeout(resolve, serviceDelay));
    return createScriptWithName(name, namespace);
  }
  return getScript(name, namespace);
};

export default {
  getScriptNameList: attachFetch(getScriptNameList, (request) => {
    return [request.params.namespace || 'user'];
  }),
  createScript: attachFetch(createScript, (request) => {
    return [
      request.body.namespace || 'user',
      request.body.scriptName || null
    ];
  }),
  deleteScript: attachFetch(deleteScript, (request) => {
    return [
      request.uriElements[request.uriElements.length-1],
      request.params.namespace || 'user'
    ];
  }),
