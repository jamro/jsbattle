...
battlefieldHelper.getTankNames = async (page) => {
  await page.waitForSelector('table.ai-table tbody tr .tank-name');
...
Given('battle quality set to {string}', async function (value) {
  await this.client.page.waitForSelector('.sim-quality-button');
  await this.client.page.click('.sim-quality-button');
  await this.client.page.waitForSelector('.sim-quality-' + value);
  await this.client.page.click('.sim-quality-' + value)
});

Given('battle speed set to {string}', async function (value) {
  await this.client.page.waitForSelector('.sim-speed-button');
  await this.client.page.click('.sim-speed-button');
  await this.client.page.waitForSelector('.sim-speed-' + value);
...
When('wait for live code widget', async function () {
  await this.client.page.waitForSelector('.live-code');
});
// THEN ------------------------------------------------------------------------
